...
Flight.fetch = function(search, deferred) {
    ...
    $.ajax({
        ...
        getFlights: function(data){
            /*
             * Method used to filter the flight while via airport is not in our database
             */
            var filtered_flights  = [],
                non_skipped_flights = [];
            _.each(data.flights, function(flights) {
                _.each(flights, function(flight) {
                    if(typeof flight.length === 'undefined') { 
                        // Only object will be pushed and blank array will be truncated
                        non_skipped_flights.push(flight);
                    }
                });
                filtered_flights.push(non_skipped_flights);
            });
