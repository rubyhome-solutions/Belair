...
<div class="ibox-title text-success">
                 <h5><i class="fa fa-wrench fa-lg"></i> Cart amendments list</h5>
...
<table class="table table-condensed table-bordered table-hover" style="max-width: 95%;">
    ...
    <?php foreach ($amendments as $amendment) { ?>
        <form class="inline" method="POST" onsubmit="document.body.style.cursor = 'wait';">
            ...
            <tr class="center">
                ...
                <td><?php echo Utils::cutMilliseconds($amendment->airBooking->created); ?></td>
                ...
                <td>
                    <?php if ($amendment->amendment_status_id === AmendmentStatus::STATUS_REQUESTED) { ?>
                        <button style="white-space: nowrap;" formaction="/airCart/processAmendment/<?php echo $amendment->group_id; ?>" type="submit" class="btn btn-info btn-xs">Click if amended with supplier</button>
                    <?php } elseif ($amendment->amendment_status_id === AmendmentStatus::STATUS_AMENDED_WITH_SUPPLIER) { ?>
                        <button style="white-space: nowrap;" formaction="/airCart/defineAmendment/<?php echo $amendment->group_id; ?>" type="submit" class="btn btn-xs btn-info">Process</button>
    ...
    <?php } ?>
                ...
                <td>
                    <?php if ($amendment->amendment_status_id === AmendmentStatus::STATUS_REQUESTED) { ?>
                        <button style="white-space: nowrap;" formaction="/airCart/abortAmendment/<?php echo $amendment->group_id; ?>" type="submit" class="btn btn-xs btn-danger">Abort</button>
    <?php } ?>
                </td>
            </tr>
            <?php if ($amendment->amendment_status_id === AmendmentStatus::STATUS_SUCCESS) { ?>
