...
// array ($airSourceId, source, destination)
    ...
    $adults = 1; //with changes in passengers count , traveler update is alos required
    $childs = 0;
    $infants = 0;
...
// echo date("Y-m-d", strtotime($date));
    ...
    foreach ($searchArray as $value) {
        ...
        $output = scrapperBooking::search($value[0], $params);

        //   sleep(10);

        if (!isset($output[0]) || in_array(substr(trim($output[0]), 1, 15), $notFound)) {
