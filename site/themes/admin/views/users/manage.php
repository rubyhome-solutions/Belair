...
//                        $("#selectedvalue").val( ui.item.value );
        ...
        ?>
        <span class="input-group-btn"><button class="btn btn btn-primary" type="submit">Submit</button></span>
    </div>
        </div>
</form>
...
if ((!empty($searchbox) && strlen($searchbox) > 2) || !empty($model->id))
    $this->widget('bootstrap.widgets.TbGridView', array(
        ...
        'columns' => array(
            ...
            array('header' => 'User', 'value' => '"<b>Name: </b>" . $data->name . "<br><b>Email: </b><a href=\'mailto:" . $data->email . "\'>". $data->email . "</a><br><b>Mobile: </b><a href=\'tel:" . $data->mobile . "\'>" . $data->mobile . "</a><br>"', 'type' => 'raw'),
            array('header' => 'User Status', 'value' => '"<b>Enabled: </b>" . ($data->enabled?"<span>Yes&nbsp;</span><a id=\'state-$data->id\' class=\'btn btn-xs btn-success\' onclick=\'switchState($data->id,0); return false;\'>Disable</a>":"<span>No&nbsp;&nbsp;</span><a id=\'state-$data->id\' class=\'btn btn-xs btn-danger\' onclick=\'switchState($data->id,1); return false;\'>Enable&nbsp;</a>") . "<br><b>Last login: </b>". $data->last_login . "<br><b>Last Transaction: </b>" . $data->last_transaction . "<br>"', 'type' => 'raw'),
...
<div class="form span6">
    ...
    <div class="clearfix">
        ...
        <button class="btn btn-primary" aria-pressed="false" data-toggle="button" onclick="$(this.parentElement.nextElementSibling).toggle();
                this.blur();
                return false;"><i class="icon-eye-close"></i>New user</button>
    
         </div>
        <div class="row">
        <div class="col-sm-10 m_top1">
        <div class="form-inline addUser">
        <fieldset class="" <?php if (!empty($searchbox)) echo "style='display:none;'" ?>>
        
        <?php
        echo $form->errorSummary($newUser);
        echo TbHtml::hiddenField('action', 'new_user');
        echo "<div class='form-group'>";
        echo $form->textFieldControlGroup($newUser, 'name',array('class'=> 'form-control'));
        echo "</div><div class='form-group'>";
        echo $form->textFieldControlGroup($newUser, 'email',array('class'=> 'form-control'));
        echo "</div><br><div class='form-group'>";
        echo $form->textFieldControlGroup($newUser, 'mobile',array('class'=> 'form-control'));
        echo '</div>';
