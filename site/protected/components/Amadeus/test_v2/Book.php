...
class Book {
    ...
    public function __construct($airSourceId, array $inputs, array $travelers, $fop, array $cc = null, $marketingCarrierId = null) {
        $this->api = new AmadeusWebServices;
        ...
        $this->airSource = \AirSource::model()->cache(120)->with('backend')->findByPk((int) $airSourceId);
        ...
        // Queueing info
        $this->airSourceQueueing = $this->airSource->getQueuingParams($inputs['segments']);

        if ($fop) {
            // Adding default form of payment
            if ($fop === \AirSource::CC_PASS_THROUGH) {    // Credit card payment
                $this->fop = new FormOfPaymentDetailsTypeI($fop);
...
//                $this->fop->creditCardCode = 'CA';
                ...
                $this->fop = new FormOfPaymentDetailsTypeI('CA');
            }
        } else {
...
//            \Utils::dbgYiiLog($resAsfr);
        ...
        if (YII_DEBUG) {
            \Utils::soapLogDebugFile($this->api, '1A_Air_SellFromRecommendation.xml');
        }
//        echo "Air_SellFromRecommendation is Confirmed!<br><br>";
        $pnrAddMultiElements = new PNR_AddMultiElements;
        $pnrAddMultiElements->pnrActions = new OptionalPNRActionsType;
...
//    $pnrAddMultiElements->pnrActions->optionCode = 10;  // 10 End transact (ET)
        ...
        $pnrAddMultiElements->dataElementsMaster->marker1 = new DummySegmentTypeI;
...
//    $infantIndicator = $this->inputs['passengers'][\TravelerType::TRAVELER_ADULT] + $this->inputs['passengers'][\TravelerType::TRAVELER_CHILD] + 1;
        foreach ($this->passengers as $passenger) {
            ...
            $travellerInfo->elementManagementPassenger = new ElementManagementSegmentType;
            $travellerInfo->elementManagementPassenger->reference = new ReferencingDetailsType_152449C;
        ...
        if (YII_DEBUG) {
            \Utils::soapLogDebugFile($this->api, '1A_PNR_AddMultiElements.xml');
        }
...
// Add the prices to the PNR
        ...
        if (YII_DEBUG) {
            \Utils::soapLogDebugFile($this->api, '1A_Fare_PricePNRWithBookingClass.xml');
        }
        ...
        if ($tmp > $this->totalAmount) {
            // Skip the pricing checks in the test API
//            $this->api->cancelCurrentPnr();
//            $this->api->Security_SignOut();
//            return ['error' => "Difference between offered and asked amounts! Offered:{$this->totalAmount} , Asked:{$tmp} , Difference:" . ($tmp - $this->totalAmount)];
        ...
        if ($this->fop) {
            ...
            if (YII_DEBUG) {
                \Utils::soapLogDebugFile($this->api, '1A_addFormOfPayment.xml');
            }
            ...
            if (YII_DEBUG) {
                \Utils::soapLogDebugFile($this->api, '1A_addTickets.xml');
            }
        }
...
//        $this->api->cancelCurrentPnr(); \Yii::app()->end();
        ...
        if (isset($this->pnrObject->pnrHeader->reservationInfo->reservation->controlNumber)) {
            ...
            if (YII_DEBUG) {
                \Utils::soapLogDebugFile($this->api, '1A_PNR_AddMultiElements_final.xml');
            }
...
//            \Utils::dbgYiiLog($this->pnrObject);
        ...
        if (!empty($this->airSourceQueueing)) {
            // Queue the pnr
            $res = $this->queuePnr();
            if (is_string($res)) {
                $this->api->Security_SignOut();
            ...
            // Issue the ticket from the queue destination airSource if auto-issue is on
            if ($this->airSourceQueueing['autoTicket'] != 0) {
                // close the current session
                $this->api->Security_SignOut();
                $issueRes = \application\components\Amadeus\Utils::issueTicket($pnr, $this->airSourceId);
//                if (YII_DEBUG) {
//                    \Utils::dbgYiiLog(['issueTicketResponse' => $issueRes]);
//                }
                if (is_string($issueRes)) {
                    return ['error' => "Ticket issue fail for PNR {$this->pnrObject->pnrHeader->reservationInfo->reservation->controlNumber} - $issueRes"];
                }
            ...
            // Issue ticket if we have fop info and autoTicket airSource setting and issue flag from the PG and there is no PRN re-queue in place
...
//            \Utils::dbgYiiLog($resTicket);
        ...
        // Get the mini rules
        $rulesApi = new \application\components\Amadeus\MiniRules($this->airSourceId, $this->api, $this->pnrObject);
        $this->rulesRaw = $rulesApi->getInfo();
...
//            $airRoute->air_booking_id = $airBooking->id;
    ...
    private function createAirCart($manualIssue) {
        ...
        $airCart->rules = $this->rulesRaw;
        $airCart->note = 'Test PNR via Amadeus V2';
        $airCart->insert();

        // Set the queueing note
        if (isset($this->airSourceQueueing['queueToPcc'])) {
            $airCart->addNote('Queued to OID: ' . $this->airSourceQueueing['queueToPcc']);
        }
...
//        \Utils::dbgYiiLog($this->passengers);
    ...
    private static function passengerData(array $passenger) {
        ...
        if (!empty($passenger['birthDate'])) {
            $passengerData->dateOfBirth = new DateAndTimeInformationType;
...
//                            \Utils::dbgYiiLog($infant->birthdate);
    ...
    private function elementPaxEmail() {
        ...
        $dataElementsIndiv->elementManagementData = new ElementManagementSegmentType;
        $dataElementsIndiv->elementManagementData->reference = new ReferencingDetailsType_152449C;
        ...
        $dataElementsIndiv->freetextData = new LongFreeTextType;
    ...
    private function elementTourCode($passengerType = 'PAX') {
        ...
        $dataElementsIndiv->tourCode = new TourCodeType;
        $dataElementsIndiv->tourCode->passengerType = $passengerType;
        $dataElementsIndiv->tourCode->netRemit = new NetRemitTourCodeType;
    ...
    private static function elementTicket($depart) {
        ...
        $dataElementsIndiv->ticketElement = new TicketElementType;
        $dataElementsIndiv->ticketElement->ticket = new TicketInformationType;
//        $dataElementsIndiv->ticketElement->ticket->indicator = 'TL';    // Ticket Time Limit (TL)
     ...
     * @return \application\components\Amadeus\test\itineraryDetails
     */
    ...
    /**
     * Queue the PNR according to airSourceQueueing parameter
    ...
    function queuePnr() {
        ...
        $request->targetDetails->targetOffice->originatorDetails->inHouseIdentification1 = $this->airSourceQueueing['queueToPcc'];
        $request->targetDetails->targetOffice->sourceType = new SourceTypeDetailsTypeI;
        $request->targetDetails->targetOffice->sourceType->sourceQualifier1 = $this->airSourceQueueing['username'] == $this->airSource->username ? 3 : 4; // 3: Place the PNR within the same office. • 4: Place the PNR in a different office.
        ...
        $request->targetDetails->queueNumber->queueDetails->number = $this->airSourceQueueing['queueNum'];
        ...
        $request->recordLocator->reservation->controlNumber = $this->pnrObject->pnrHeader->reservationInfo->reservation->controlNumber;
        ...
        $res = $this->api->Queue_PlacePNR($request);
        if (YII_DEBUG) {
            \Utils::dbgYiiLog(['Queue_PlacePNR' => $res]);
        }
        if (isset($res->errorReturn->errorText->freeText)) {
            return "PNR Queueing failed for PNR {$this->pnrObject->pnrHeader->reservationInfo->reservation->controlNumber} - {$res->errorReturn->errorText->freeText}";
        }
