...
namespace application\components\Flydubai\test;
set_time_limit(100);
...
class Flydubai extends \SoapClient{
	...
	const BASE_URL='http://uat.ops.connectpoint.flydubai.com/';
...
//        }
   ...
   <soapenv:Body>
     ...
     \Utils::dbgYiiLog(['TEST RetrieveSecurityToken', $strXml]);
            $soapAction="\"http://tempuri.org/IConnectPoint_Security/RetrieveSecurityToken\"";
        $res = $this->curl(self::BASE_URL.'ConnectPoint.Security.svc', $strXml,$soapAction);
...
\Utils::dbgYiiLog(['TEST LoginTravelAgent', $strXml]);
        $soapAction="\"http://tempuri.org/IConnectPoint_TravelAgents/LoginTravelAgent\"";       
        $res = $this->curl(self::BASE_URL.'ConnectPoint.TravelAgents.svc',$strXml,$soapAction);
  ...
  \Utils::dbgYiiLog(['TEST RetrieveAgencyCommission', $strXml]);
...
//        $departure_date = str_replace('-', '', $departure_date);
  ...
  \Utils::dbgYiiLog(['TEST RetrieveFareQuote', $strXml]);
         $soapAction="\"http://tempuri.org/IConnectPoint_Pricing/RetrieveFareQuote\"";    
         $res = $this->curl(self::BASE_URL.'ConnectPoint.Pricing.svc', $strXml,$soapAction);
  \Utils::dbgYiiLog($res);
if(!empty($res['error'])){
          \Utils::dbgYiiLog(__METHOD__ . " Error: " . $res['error']);    // Log the result as error
            return false;
