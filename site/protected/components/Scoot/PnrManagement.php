...
//    private $testMe = true;
    ...
    /**
     * Scoot Session manager
     * @var SessionManager
     */
    public $apiSessionManager = null;

    /**
     * Scoot Booking manager
    ...
    function addPax($passenger) {
        ...
        $title=Utils::createScootTitle($passenger['title'], $passenger['type']);
        if ($passenger['type'] != \TravelerType::TRAVELER_INFANT) {
            ...
            //\Utils::YiidbgLog($dob);
            //\Utils::dbgYiiLog(['passengerPax'=>$passenger]);
            
            $weightCategory=Utils::setWeightCategory($passenger['title'], $passenger['type']);
            $pax = new Passenger;
            ...
            $pax->Names[0]->Title = $title;
            $pax->Names[0]->State = MessageState::aNew;
            $pax->PassengerInfo =  new PassengerInfo;
            $pax->PassengerInfo->Gender = (strstr($passenger['title'], 'Ms.')|| strstr($passenger['title'], 'Mrs.')) ? Gender::Female : Gender::Male;
            $pax->PassengerInfo->WeightCategory = $weightCategory;
...
//            $pax->Infant = new PassengerInfant;
            ...
            $pi->Names[0]->Title = $title;
            $this->passengers[count($this->infantSsrs) - 1]->PassengerInfants[] = $pi;
        }
    }

    function addCashFop($fareDiff=0) {
        if (!$this->api) {
            return "The TZ PNR Management is not connected";
        }
        ...
        $ap->addPaymentToBookingReqData->QuotedCurrencyCode = Utils::currencyFinderScoot($this->origin_aiport_code);
        ...
        if($this->payAmount==0){
            $ap->addPaymentToBookingReqData->QuotedAmount = Utils::currencyConvertorFromINR($this->origin_aiport_code, (float) $this->faresTotal+(float)$fareDiff);
        }else{
            $ap->addPaymentToBookingReqData->QuotedAmount = $this->payAmount;
        }
        ...
        if (YII_DEBUG) {
            $res->asXML('Scoot_AddPaymentToBooking_response.xml');
        }
    ...
    function addAirSegments(array $segments) {
        ...
        foreach ($segments as $segment) {
           ...
           //$ar->FareClasses[] = 'B2';
            //$ar->ProductClasses[] = 'J';
            if($firstFlag){
                $this->origin_aiport_code=$first['origin'];
                $firstFlag=false;
            }
            ...
            $ar->CurrencyCode =  Utils::currencyFinderScoot($this->origin_aiport_code);
//            $ar->IncludeTaxesAndFees = 'true';
            $this->availabilityRequests[] = $ar;
            // If there are infants
            if (!empty($this->infantSsrs)) {
                if (count($segment) > 1 && $first['flightNumber']!= $last['flightNumber']) {
                    foreach ($segment as $csegment) {
                        $this->segmentSSRRequests[] = $this->createSegmentSsrRequest($csegment['departTs'], $csegment['origin'], $csegment['destination'], $csegment['marketingCompany'], $csegment['flightNumber']);
                    }
                } else {
                    $this->segmentSSRRequests[] = $this->createSegmentSsrRequest($first['departTs'], $first['origin'], $last['destination'], $first['marketingCompany'], $first['flightNumber']);
                }
    ...
    function checkAvailability() {
        ...
        foreach ($this->availabilityRequests as $ar) {
            if($this->promocode !== null){
              $ar->PromotionCode = $this->promocode;   
            }
...
//            $response = $this->api->GetAvailability($request);
            $xmlResponse = \simplexml_load_file('scoot_response.xml');
        } else {
            ...
            if (YII_DEBUG) {
                $xmlResponse->asXML('scoot_response.xml');
            }
        }
...
//        print_r($strXml);
        foreach ($strXml->Body->GetAvailabilityByTripResponse->GetTripAvailabilityResponse->Schedules->ArrayOfJourneyDateMarket as $ajdm) {
            ...
            foreach ($ajdm->JourneyDateMarket->Journeys->Journey as $journey) {
                ...
                if (count($journey->Segments->Segment) >= 2) {
                    $faresellkey = '';
                    $seg_counter = 1;
                    //\Utils::dbgYiiLog(['segmentsss'=>$journey]);
                    //echo $segment;
                    foreach ($journey->Segments->Segment as $seg) {
                        if(isset($seg->Legs->Leg[0])){
                         foreach($seg->Legs->Leg as $leg){
                            $allLegs[0][] = $leg;
                         }   
                        }else{
                ...
                //\Utils::dbgYiiLog(['allLegs'=>$allLegs]);
                //echo $segment;
     ...
     * @return \application\components\Scoot\SegmentSSRRequest
     */
    private function createSegmentSsrRequest($departureTs, $origin, $destination, $carrier, $flightNumber) {
    ...
    function checkFares($faresTotal) {
        ...
        $ipr->SellByKeyRequest->CurrencyCode = Utils::currencyFinderScoot($this->origin_aiport_code);
...
//        print_r($ipr);
        if (!$this->testMe) {
            ...
            $res = $this->api->toSimpleXmlElement();
            if (YII_DEBUG) {
                $res->asXML('scoot_GetItineraryPrice_response.xml');
            }
        } else {
            $res = simplexml_load_file('scoot_GetItineraryPrice_response.xml');
        }
//        print_r($res);
        //\Utils::dbgYiiLog($res->Body->PriceItineraryResponse->Booking->BookingSum->TotalCost);
        $this->payAmount = (float) $res->Body->PriceItineraryResponse->Booking->BookingSum->TotalCost;
        $sum = Utils::currencyConvertorToINR($this->origin_aiport_code, (float) $res->Body->PriceItineraryResponse->Booking->BookingSum->TotalCost);
        // \Utils::dbgYiiLog(['sum'=>$sum,'faresTotal'=>$this->faresTotal,'diff'=>$sum - $this->faresTotal]);
//        if (abs($sum - $this->faresTotal)> 2) {
//            return $sum - $this->faresTotal;
//        } else {
//            return true;
//        }
        if ($sum <= $this->faresTotal || ($sum - $this->faresTotal) < 1) {
            $this->faresTotal = $sum;
            //\Utils::dbgYiiLog($this->faresTotal);
            return true;
        } else {
            return $sum - $this->faresTotal;
        }
    ...
    private function sellRequest() {
        ...
        $sr->SellRequestData->SellJourneyByKeyRequest->SellJourneyByKeyRequestData->CurrencyCode = Utils::currencyFinderScoot($this->origin_aiport_code);
        ...
        if (!$this->testMe) {
            ...
            $res = $this->api->toSimpleXmlElement();
            if (YII_DEBUG) {
                $res->asXML('scoot_SellRequest1_response.xml');
            }
        } else {
            $res = simplexml_load_file('scoot_SellRequest1_response.xml');
        }
//        print_r($res);
        ...
        if (!empty($this->segmentSSRRequests)) {
            ...
            $sr->SellRequestData->SellSSR->SSRRequest->CurrencyCode = Utils::currencyFinderScoot($this->origin_aiport_code);
...
//            print_r($res);
        ...
        //\Utils::dbgYiiLog(['sellKey'=>$res->Body->PriceItineraryResponse->Booking->BookingSum->TotalCost]);
        $sum = Utils::currencyConvertorToINR($this->origin_aiport_code, $sum);
//       if (($sum - $this->faresTotal)>2) {
...
//        }
    ...
    private function bookingContact() {
        ...
        $bc->TypeCode = "p";
        $bc->EmailAddress = $details->email;
        $bc->HomePhone = $details->phone;
        $bc->OtherPhone= $details->phone;
        $bc->AddressLine1 = empty($details->address1)?'XYZABC':$details->address1;
        $bc->AddressLine2 = $details->address2;
        $bc->City = $details->city;
        $bc->ProvinceState = $details->stateCode;
        $bc->PostalCode = empty($details->pincode)?'110025':$details->pincode;
    ...
    private function bookCommit() {
        ...
        //\Utils::dbgYiiLog($this->bookingContact());
//        if (empty($this->passengers)) {
//            return 'The passengers are not added';
//        }
//        \Utils::dbgYiiLog('bookcommit');
        ...
        $bcr->BookingCommitRequestData->CurrencyCode =  Utils::currencyFinderScoot($this->origin_aiport_code);
...
//        print_r($res);
        if (YII_DEBUG) {
            $res->asXML('scoot_BookingCommit_response.xml');
        }
...
//            print_r($this->pnrResponseObj);
        ...
        $check = $this->addCashFop($fareDiff);
        if ($check !== true) {
            $this->apiSessionManager->Logout();
            return ["error" => $check];
        }
...
//        print_r($res);
        ...
        $this->faresTotal = Utils::currencyConvertorToINR($this->origin_aiport_code, (float) $res->Body->CancelResponse->BookingUpdateResponseData->Success->PNRAmount->BalanceDue);
        $check = $this->addCashFop();
        if ($check !== true) {
            return $check;
        }
