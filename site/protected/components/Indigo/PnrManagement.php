...
//    private $testMe = true;
    ...
    /**
     * Indigo Session manager
     * @var SessionManager
     */
    public $apiSessionManager = null;

    /**
     * Indigo Booking manager
...
//            $pax->Infant = new PassengerInfant;
    ...
    function addCashFop() {
        if (!$this->api) {
            return "The 6E PNR Management is not connected";
        }
        ...
        $ap->addPaymentToBookingReqData->Installments = 1;  // Indigo change made on 11.Aug.2015
        $this->api->AddPaymentToBooking($ap);
        $res = $this->api->toSimpleXmlElement();
        if (YII_DEBUG) {
            $res->asXML('6E_AddPaymentToBooking_response.xml');
        }
//        print_r($res);
...
//            $response = $this->api->GetAvailability($request);
            $xmlResponse = \simplexml_load_file('6E_response.xml');
        } else {
            ...
            if (YII_DEBUG) {
                $xmlResponse->asXML('6E_response.xml');
            }
        }

//        \Utils::dbgYiiLog($this->originalSegments); exit;
...
//                echo "Found: " . print_r($match, true);
        ...
        foreach ($strXml->Body->GetAvailabilityByTripResponse->GetTripAvailabilityResponse->Schedules->ArrayOfJourneyDateMarket as $ajdm) {
            ...
            foreach ($ajdm->JourneyDateMarket->Journeys->Journey as $journey) {
                ...
                if (count($journey->Segments->Segment) == 2) {
                    $allLegs[0] = [
                        $journey->Segments->Segment[0]->Legs->Leg,
                        $journey->Segments->Segment[1]->Legs->Leg,
                    ];
                    $faresellkey = $journey->Segments->Segment[0]->Fares->Fare->FareSellKey . "^" . $journey->Segments->Segment[1]->Fares->Fare->FareSellKey;
                } else {
     ...
     * @return \application\components\Indigo\SegmentSSRRequest
     */
...
//        print_r($ipr);
        if (!$this->testMe) {
            ...
            //\Utils::dbgYiiLog($res);
            if (YII_DEBUG) {
                $res->asXML('Indigo_GetItineraryPrice_response.xml');
            }
        } else {
            $res = simplexml_load_file('Indigo_GetItineraryPrice_response.xml');
        }
    //        print_r($res);
...
//        }
    ...
    private function sellRequest() {
        ...
        if (!$this->testMe) {
            ...
            $res = $this->api->toSimpleXmlElement();
            if (YII_DEBUG) {
                $res->asXML('Indigo_SellRequest1_response.xml');
            }
        } else {
            $res = simplexml_load_file('Indigo_SellRequest1_response.xml');
        }
//        print_r($res);
        ...
        if (!empty($this->segmentSSRRequests)) {
            ...
            if (!$this->testMe) {
                ...
                $res = $this->api->toSimpleXmlElement();
                if (YII_DEBUG) {
                    $res->asXML('Indigo_SellRequest2_response.xml');
                }
            } else {
                $res = simplexml_load_file('Indigo_SellRequest2_response.xml');
            }
//            print_r($res);
            if (!empty($res->Body->SellResponse->BookingUpdateResponseData->Error)) {
                return (string) $res->Body->SellResponse->BookingUpdateResponseData->Error;
...
//        print_r($res);
        if (YII_DEBUG) {
            $res->asXML('Indigo_BookingCommit_response.xml');
        }
...
//            ],
        ...
        //[JourneySellKey] => 6E~2441~ ~~DEL~12/29/2014 07:50~BOM~12/29/2014 12:10~JLR
...
//        return $this->createPnr();
    ...
    function checkAvailabilityAndFares() {
        ...
        $check = $this->checkFares(null);
        if ($check !== true) {
//        if ($check !== true && $this->airSourceId != Utils::TEST_AIRSOURCE_ID) {
            $this->apiSessionManager->Logout();
            return [
...
//        if ($check !== true && $this->airSourceId != Utils::TEST_AIRSOURCE_ID) {
    ...
    function retrievePnr($pnrStr) {
        ...
        return "Indigo PNR management is not initialized";
    }
