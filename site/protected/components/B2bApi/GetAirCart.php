...
class GetAirCart {
    ...
    function validate() {
        ...
        if (empty($this->cartID)) {
            throw new B2bApiException(B2bApiException::MISSING_CART_ID, null, 400);
        }
        return true;
    }
    ...
    function setCart() {
        ...
        if ($this->airCart === null) {
            throw new B2bApiException(B2bApiException::CART_NOT_FOUND, null, 404);
        }
        if ($this->airCart->user->user_info_id !== $this->credentials->user->user_info_id) {
            throw new B2bApiException(B2bApiException::CART_NOT_OWNED, null, 403);
        }
    }
