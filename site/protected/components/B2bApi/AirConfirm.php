...
//        $check = true;
        ...
        throw new B2bApiException($check['errorCode'], $check['message'], 400, $check['priceDiff']);
    }
    ...
    static function fetchTheRcs(array $arrRcIds, $ts) {
        ...
        foreach ($arrRcIds as $journey) {
            ...
            foreach ($journey as $rcId) {
                $rc = \RoutesCache::model()->findByPk((int) $rcId);
                /* @var $rc \RoutesCache */
                if ($rc === null) { // RC not found
                    throw new B2bApiException(B2bApiException::RC_NOT_FOUND, null, 404, "RC element# $rcId not found");
                }
                if (strtotime($rc->updated) > $ts) {   // RC element is updated after the TS from the search key
                    throw new B2bApiException(B2bApiException::REQUEST_OUT_OF_DATE, null, 400, "RC# $rc->id, $rc->updated, $ts, " . time($rc->updated));
                }
                if (strtotime($rc->departure_date . ' ' . $rc->departure_time) < time()) {   // RC element is for flight in the past
                    throw new B2bApiException(B2bApiException::REQUEST_OUT_OF_DATE, null, 400, "RC# $rc->id, $rc->departure_date $rc->departure_time You can not book flight in the past");
                }
                $tmp[] = $rc;
            }
            $out[] = $tmp;
