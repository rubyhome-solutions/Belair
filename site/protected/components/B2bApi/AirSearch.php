...
class AirSearch {
    ...
    function validate() {
        ...
        if (strlen($this->origin) !== 3 || strlen($this->destination) !== 3 || empty($this->onwardDate)) {
            throw new B2bApiException(B2bApiException::SEARCH_FIELDS_MALFORMED, null, 400);
        }
        // Validate dates
        $onward = strtotime($this->onwardDate);
        if ($onward < strtotime(date(DATE_FORMAT))) {
            throw new B2bApiException(B2bApiException::ONWARD_DATE_IN_PAST, null, 400);
        }
        if (!empty($this->returnDate)) {
            $back = strtotime($this->returnDate);
            if ($back < $onward) {
                throw new B2bApiException(B2bApiException::WRONG_RETURN_DATE, null, 400);
            }
        }
        // Validate Paxes
        if ($this->adults == 0 && ($this->children == 0 || $this->infants > 0)) {
            throw new B2bApiException(B2bApiException::INFANT_WITHOUT_ADULT, null, 400);
        }
