...
class Utils {
    ...
    const PRODUCTION_AIRSOURCE_ID = 32;
    const TEST_AIRSOURCE_ID = 24;
    ...
    // Fares and classes declare here
    const FARE_TYPES = 'HB';
    const PRODUCT_CALSSES = 'HO';
    const SSRCode = 'HBAG';
    const LITE_FARE = 'LITE';
    const REGULAR_FARE = 'REGULAR';

    static $productClass = [
        'RS' => self::REGULAR_FARE,
        'HO' => self::LITE_FARE
    ];
...
//        $difference = 0;
    ...
    static function search($airSourceId, \stdClass $params) {
        ...
        if ($params->category != \CabinType::ECONOMY) {
            return "SG can serve only economy class";
        }
        ...
        if ($airSource->spare2 === self::LITE_FARE) {
            $availabilityRequest->FareTypes[] = self::FARE_TYPES;
            $availabilityRequest->ProductClasses[] = self::PRODUCT_CALSSES;
        }
        ...
        if (!empty($params->return)) {
            ...
            //check for only lite Fare class for return trip
            if ($airSource->spare2 === self::LITE_FARE) {
                $availabilityRequest->FareTypes[] = self::FARE_TYPES;
                $availabilityRequest->ProductClasses[] = self::PRODUCT_CALSSES;
            }
        ...
        $debugFilename = \Yii::app()->runtimePath . DIRECTORY_SEPARATOR . 'spicejet_GetAvailability_response.json';
        if (!$test) {
...
//        \Utils::dbgYiiLog($arr['GetTripAvailabilityResponse']['Schedules']['ArrayOfJourneyDateMarket']);
        ...
        //\Utils::dbgYiiLog(['availablityResponse'=>$arr]);
        // Price info
...
//            }
        ...
        //\Utils::dbgYiiLog(['ItineraryRes'=>$itineraryPriceResponse]);
        $debugFilename = \Yii::app()->runtimePath . DIRECTORY_SEPARATOR . 'spicejet_GetItineraryPrice_response.json';
        if (!$test) {
//            $pir = new PriceItineraryRequest($itineraryPriceRequest);
...
//                }
        ...
        //\Utils::dbgYiiLog(['journeys'=>$journeys]);
        //\Yii::app()->end();
        foreach ($journeys as $jorney) {
            if (isset($jorney['Segments']['Segment'][0])) {
                ...
                $lastSegment = end($jorney['Segments']['Segment']);
                $arrivingStation = $lastSegment['ArrivalStation'];
                $departureStation = $firstSegment['DepartureStation'];
                $firstSegmentFareKey = $firstSegment['Fares']['Fare']['FareSellKey'] . '~' . $departureStation . $arrivingStation;
                //unset($legs);
                foreach ($jorney['Segments']['Segment'] as $segment) {

                    $fareKey = $segment['Fares']['Fare']['FareSellKey'] . '~' . $departureStation . $arrivingStation;
                    $legs[$seg_counter - 1] = $segment['Legs']['Leg'];
                    if ($seg_counter === 1) {
                        if (isset($segment['Fares']['Fare']['PaxFares']['PaxFare'][0])) {
                            $passengers = $segment['Fares']['Fare']['PaxFares']['PaxFare'];
                        } else {
                            unset($passengers);
                            $passengers[0] = $segment['Fares']['Fare']['PaxFares']['PaxFare'];
                        }
                    }

                    if ($seg_counter > 1 && isset($fares[$fareKey . 'segment' . $seg_counter]['baseAmount']) && isset($fares[$firstSegmentFareKey])) {
                        $fares[$firstSegmentFareKey]['baseAmount'] = $fares[$firstSegmentFareKey]['baseAmount'] + $fares[$fareKey . 'segment' . $seg_counter]['baseAmount'];
                    }
                    $seg_counter++;
                }
            } else {
            ...
            $rows = array_merge($rows, self::prepareCacheRow($legs, $passengers, $fares, $airSourceId, $serviceTypeId, $fsk, $params));
            $resultCount++;
            if ($resultCount > 99) {
...
//            echo \Utils::dbg($legs);
    ...
    static function prepareLegsJson($legs, $bookingClass, $productClass = null) {
        ...
        foreach ($legs as $leg) {
            if (isset($leg[0])) {
                $firstLeg = $leg[0];
                $lastLeg = $leg[1];
                $arrival_time = explode('T', $firstLeg['STA']);
                $departure_time = explode('T', $lastLeg['STD']);
                //$technicalStop=null;
                $technicalStop = $lastLeg['DepartureStation'] . ' ' . $arrival_time[1] . '-' . $departure_time[1];
            } else {
                $firstLeg = $leg;
                $lastLeg = $leg;
                $technicalStop = null;
            ...
            $legsJson->arrive = self::shortenDateAndTime($lastLeg['STA']);
            $legsJson->depart = self::shortenDateAndTime($firstLeg['STD']);
            $legsJson->destination = \Airport::getAirportCodeAndCityNameFromCode($lastLeg['ArrivalStation']);
            $legsJson->destinationTerminal = $lastLeg['LegInfo']['ArrivalTerminal'];
            $legsJson->flighNumber = $firstLeg['FlightDesignator']['CarrierCode'] . '-' . trim($firstLeg['FlightDesignator']['FlightNumber']);
            $legsJson->origin = \Airport::getAirportCodeAndCityNameFromCode($firstLeg['DepartureStation']);
            $legsJson->originTerminal = $firstLeg['LegInfo']['DepartureTerminal'];
            $legsJson->time = \Utils::convertSecToHoursMins(strtotime($lastLeg['STA']) - strtotime($firstLeg['STD']));
            $legsJson->bookingClass = $bookingClass;
            $legsJson->aircraft = $firstLeg['LegInfo']['EquipmentType'];
            $legsJson->product_class = $productClass;
            $legsJson->ts = $technicalStop;
            $out[] = $legsJson;
        }
        return $out;
    }
    ...
    static function prepareHashStr($airSourceId, $legs, $cabinTypeId, $productClass = null) {
        ...
        foreach ($legs as $leg) {
            ...
            // \Utils::dbgYiiLog(['firstLeg'=>$firstLeg]);
            $out .= $firstLeg['DepartureStation'] . \RoutesCache::HASH_SEPARATOR;
            $out .= self::shortenDateAndTime($firstLeg['STD']) . \RoutesCache::HASH_SEPARATOR;
            $out .= $firstLeg['FlightDesignator']['CarrierCode'] . '-' . trim($firstLeg['FlightDesignator']['FlightNumber']) . \RoutesCache::HASH_SEPARATOR;
            $out .= $lastLeg['ArrivalStation'] . \RoutesCache::HASH_SEPARATOR;
            $out .= $productClass . \RoutesCache::HASH_SEPARATOR;
        }
        return $out;
    }

    static function prepareCacheRow($legs, $passagers, $fares, $airSourceId, $serviceTypeId, $fsk, $params) {
        $processedPaxTypes = [];
        $rows = [];
        ...
        foreach ($passagers as $passager) {
            ...
            // Taxes
            $row->total_taxes = $fares[$tmpFsk]['amount'];
            $row->base_fare = $fares[$tmpFsk]['baseAmount'];
    ...
    static function aquirePnr($pnrStr, $airSourceId) {
        $airSource = \AirSource::model()->with('backend')->findByPk($airSourceId);
        $api = new \application\components\Spicejet\PnrManagement;
        ...
        if (YII_DEBUG) {
            \Utils::objectToFile('spicejet_pnr_structure.xml', $api->pnrResponseObj);
        }
        $acquisition = new \application\components\Spicejet\PnrAcquisition($api->pnrResponseObj, $airSourceId);
