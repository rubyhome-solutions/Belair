...
class TourCodeController extends Controller {
    ...
    /**
     * Creates new TourCode record.
     */
    public function actionCreate() {
        $model = new TourCode;
        if (isset($_POST['TC'])) {
            $model->attributes = $_POST['TC'];
    ...
    public function actionAdmin() {
        $model = new TourCode('search');
        $model->unsetAttributes();  // clear any default values
        if (isset($_GET['TourCode'])) {
            Yii::app()->session->add('filterTourCode', $_GET['TourCode']);
        }
        $model->attributes = Yii::app()->session->get('filterTourCode', []);
        ...
        if (Yii::app()->request->isAjaxRequest && Yii::app()->request->getQuery('ajax') === 'tour-code-grid') {
    ...
    public function loadModel($id) {
        $model = TourCode::model()->findByPk($id);
        if ($model === null) {
