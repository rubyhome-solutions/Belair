...
//      throw new CException("Invalid IP address: " . var_export($addr, true), self::ERR_INVALID_IP);
            ...
            $record->set('dmaCode', "Invalid IP address: " . print_r($addr, true));
            return $record;
        }
...
//            throw new CException("Invalid database type; lookupLocation() method expects City database.");
            $record = new GeoIP_Location();
            $record->set('dmaCode', "Invalid database type; lookupLocation() method expects City database.");
            return $record;
        }
    ...
    protected function getRecord($ipnum) {
        ...
        if (!isset(self::$COUNTRY_CODES[$char]) || !isset(self::$COUNTRY_CODES3[$char]) || !isset(self::$COUNTRY_NAMES[$char])) {
            \Utils::dbgYiiLog([
                'bad ipnum' => $ipnum,
                'char' => $char
            ]);
