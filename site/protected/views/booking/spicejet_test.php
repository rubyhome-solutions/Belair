...
$test = true;
if (!$test) {
    $apiSesMan = new \application\components\Spicejet\SessionManager([], 'test');
    ...
    $bookingApi = new \application\components\Spicejet\BookingManager($signature, [], 'test');
} else {
    $model->adults = 2;
    $model->infants = 1;
    $model->children = 1;
...
if ($activeCompanyId) {
    ...
    if ($test) {   // fill the treavelers and cc fake test data
...
if (!$test) {
    ...
    if (YII_DEBUG) {
        file_put_contents('spicejet_GetAvailability_response.txt', json_encode($response));
    }
} else {
    $response = json_decode(file_get_contents('spicejet_GetAvailability_response.txt'));
}
...
if (!$test) {
    ...
    if (YII_DEBUG) {
        file_put_contents('spicejet_GetItineraryPrice_response.json', json_encode($itineraryPriceResponse));
    }
} else {
    $itineraryPriceResponse = json_decode(file_get_contents('spicejet_GetItineraryPrice_response.json'));
}
