<h3>Indigo production results</h3>
<?php
...
$test = false;
//\Utils::dbgYiiLog("FIRST marker " . round(microtime(true) - $start_marker, 2) . " sec.");
//$start_marker = microtime(true);
if (!$test) {
    $apiSesMan = new \application\components\Indigo\SessionManager([], 'production');
//    \Utils::dbgYiiLog("After SessionManager INIT marker " . round(microtime(true) - $start_marker, 2) . " sec.");
//    $start_marker = microtime(true);
    $signature = $apiSesMan->Logon(application\components\Indigo\Utils::PRODUCTION_AIRSOURCE_ID);
//    \Utils::dbgYiiLog("After LOGON INIT marker " . round(microtime(true) - $start_marker, 2) . " sec.");
//    $start_marker = microtime(true);
    if ($signature === false) {   // Error
        Utils::finalMessage('Error with Indigo. Please check the logs for details!');
    }
    $bookingApi = new \application\components\Indigo\BookingManager($signature, [], 'production');
//    \Utils::dbgYiiLog("After BookingManager INIT marker " . round(microtime(true) - $start_marker, 2) . " sec.");
    $start_marker = microtime(true);
} else {
    $model->adults = 1;
    $model->infants = 1;
...
//\Utils::dbgYiiLog("POST LOGON marker " . round(microtime(true) - $start_marker, 2) . " sec.");
//$start_marker = microtime(true);
