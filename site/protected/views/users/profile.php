...
$canSetPrmissions = (\Authorization::can(\Authorization::SET_USER_PERMISSIONS, ['user_id' => $model->id]) === true);
?>
...
//                        Yii::log($model->city_id);
            echo TbHtml::dropDownList('country', !empty($model->city_id) ? $model->city->state->country_id : '100100', CHtml::listData(Country::model()->findAll(), 'id', 'name'), array('style' => 'width: 75%; float:right;', 'onchange' => 'getStates(false)'));
            ?>
        </div>
        <div class="row-fluid">
...
//        Yii::log(print_r($permissions, true));
            foreach (Permission::model()->findAllByAttributes(['staff_only' => $model->isStaff ? 1 : 0], ['order' => 'id']) as $permission) {
                echo TbHtml::checkBox('Permission[' . $permission->id . ']', in_array($permission->id, $permissions), ['style' => 'float:left; margin-left: 10%;']);
