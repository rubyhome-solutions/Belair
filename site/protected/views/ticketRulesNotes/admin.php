<?php
/* @var $this TicketRulesNotesController */
/* @var $model TicketRulesNotes */

$this->breadcrumbs=array(
	'Ticket Rules Notes'=>array('admin'),
	'Manage',
);

$this->menu=array(
	array('label'=>'Create TicketRulesNotes', 'url'=>array('create')),
);

Yii::app()->clientScript->registerScript('search', "
$('.search-button').click(function(){
	$('.search-form').toggle();
	return false;
});
$('.search-form form').submit(function(){
	$('#ticket-rules-notes-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
");
?>

<h1>Manage Ticket Rules Notes</h1>

<p>
You may optionally enter a comparison operator (<b>&lt;</b>, <b>&lt;=</b>, <b>&gt;</b>, <b>&gt;=</b>, <b>&lt;&gt;</b>
or <b>=</b>) at the beginning of each of your search values to specify how the comparison should be done.
</p>

<?php echo CHtml::link('Advanced Search','#',array('class'=>'search-button')); ?>
<a href='/ticketRulesNotes/create' class='btn primary blue' style='float:right'>Create</a>
<div class="search-form" style="display:none">
<?php $this->renderPartial('_search',array(
	'model'=>$model,
)); ?>
</div><!-- search-form -->

<?php $this->widget('zii.widgets.grid.CGridView', array(
	'id'=>'ticket-rules-notes-grid',
	'dataProvider'=>$model->search(),
	'filter'=>$model,
	'columns'=>array(
		//'id',
           //     'note_id',
            [
            'name' => 'note_id',
            'headerHtmlOptions' => ['style' => 'width: 60px;'],
            ],
		'airline_code',
		'iata_on_basic',
		'instructions',
		'created',
		'airline_with_remarks',
		array(
			'class'=>'CButtonColumn',
		),
	),
)); ?>
